...
# Decompiled from: Python 3.12.7 (tags/v3.12.7:0b05ead, Oct  1 2024, 03:06:41) [MSC v.1941 64 bit (AMD64)]
# Embedded file name: C:\Users\fsociety\Desktop\unpack\MY_scripts_unpacked\scripts - client\connection_mgr.py
# Compiled at: 2026-06-21 15:10:10
...
class ConnectionManager(IConnectionManager):
    ...
    def __tryConnect(self):
        ...
        LOG_NOTE('[LOCAL SERVER] BigWorld.connect URL: %s' % self.__connectionUrl)
        LOG_NOTE('[LOCAL SERVER] publicKeyPath: %s' % self.__connectionData.publicKeyPath)
        LOG_NOTE('[LOCAL SERVER] username: %s' % self.__connectionData.username)
        LOG_NOTE('[LOCAL SERVER] inactivityTimeout: %s' % self.__connectionData.inactivityTimeout)
        try:
            BigWorld.connect(self.__connectionUrl, self.__connectionData, self.__serverResponseHandler)
            LOG_NOTE('[LOCAL SERVER] BigWorld.connect returned OK')
        except Exception as e:
            LOG_NOTE('[LOCAL SERVER] BigWorld.connect FAILED: %s' % str(e))

        return

    @uniprof.regionDecorator(label='offline.connect', scope='exit')
    def __serverResponseHandler(self, stage, status, responseDataJSON):
        LOG_NOTE('[LOCAL SERVER] Response: stage=%s status=%s data=%s' % (stage, status, responseDataJSON))
        status = str(status)
        self.__connectionInProgress = False
    ...
    def __setConnectionData(self, params, password):
        ...
        self.__connectionData.clientContext = json.dumps({'lang_id': (getClientLanguage()), 'publication': (params.get('publication'))})
...
# okay decompiling C:\Users\fsociety\Desktop\unpack\MY_scripts_unpacked\scripts - client\connection_mgr.pyc
